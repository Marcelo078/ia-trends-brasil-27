  ...
  return <header className="sticky top-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
      ...
      {isMenuOpen && <div className="md:hidden border-t bg-background">
          <div className="container mx-auto px-4 py-4 space-y-4">
            <Link to="/" className="block text-foreground hover:text-brasil-blue transition-colors" onClick={() => setIsMenuOpen(false)}>
              Início
            </Link>
            <Link to="/categorias" className="block text-foreground hover:text-brasil-blue transition-colors" onClick={() => setIsMenuOpen(false)}>
              Categorias
            </Link>
            <Link to="/sobre" className="block text-foreground hover:text-brasil-blue transition-colors" onClick={() => setIsMenuOpen(false)}>
              Sobre
            </Link>
            <Button asChild variant="default" className="w-full bg-gradient-to-r from-brasil-orange to-brasil-yellow">
              <a href="https://lovable.dev/?via=marcelo-barbosa" target="_blank" rel="noopener noreferrer">
                <Zap className="w-4 h-4 mr-2" />
                Ferramentas IA
              </a>
